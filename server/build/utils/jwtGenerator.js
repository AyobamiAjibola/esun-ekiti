...
const jwtGenerator = (user) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const userToken = yield UserToken_1.default.findOne({ where: { UserId: user.id } });
        if (userToken)
            yield UserToken_1.default.destroy({ where: { UserId: user.id } });
        const expired = new Date();
        expired.setDate(expired.getDate() + 7);
        yield UserToken_1.default.create({
            UserId: user.id,
            token: refreshToken,
            expired_at: expired
        });
