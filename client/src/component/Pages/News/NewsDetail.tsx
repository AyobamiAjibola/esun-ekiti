...
const fetchComment = async (pageParam: number, identifier: string) => {
  const res = await axios.get(`comment/fetch/news/${identifier}?page=${pageParam}&size=3`)
  return res.data;
};
...
export default function NewsDetail () {
  ...
  const onSubmitLike = async (idx: number, commentId: number) => {
    ...
    try {
      ...
      await axios.put(`comment/news/like/${id}`, requestBody);
  ...
  //     await axios.put(`comment/news/dislike/${id}`, requestBody);
          ...
          : status === 'error'
            ? ( //eslint-disable-line
                ...
                >
                    ...
                    <Box>
                      ...
                      <Box
                        sx={{
                          ...
                          float: 'left',
                          mr: 2, my: 2,
                          display: newsData?.data?.data?.singleNews?.videoLink ? 'block' : 'none'
                        }}
                      >
                        <YouTubeVideo
                          videoLink={newsData?.data?.data?.singleNews?.videoLink}
                          title={newsData?.data?.data?.singleNews?.videoTitle}
                          width='100%'
                          height= '300px'
                        />
                      </Box>
                  ...
                  <Box sx={{width: '95%', height: '0.2px', backgroundColor: '#CCCCCC', mt: 3, mb:2}}/>
                  ...
                  >
                    <Typography sx={{fontWeight: 600, color: '#585858'}}>{commentTotal} Comments</Typography>
...
function New_Comment ({id, setUpdate}: Props_Comment) {
  ...
  const onSubmit: SubmitHandler<Inputs> = async (data: any) => {
    try {
      ...
      const res = await axios.post(`comment/comment_news/${id}`, requestBody);
...
function New_Reply ({id, setUpdate}: Props_Reply) {
  ...
  const onSubmit: SubmitHandler<Inputs_Reply> = async (data: any) => {
    try {
      ...
      const res = await axios.post(`comment/reply/news/${id}`, requestBody);
...
function Replies ({replies, id, setUpdate, loadMore, handleToggleShowReply}: Props) {
  ...
  //     await axios.put(`comment/reply/news/like/${id}`, requestBody);
  ...
  //     await axios.put(`comment/reply/news/dislike/${id}`, requestBody);
                ...
                <Box
                  ...
                  <IconButton sx={{ml: 1}}
                    onClick={() => {
                      ...
                      sessionStorage.setItem('commentId', value.commentNewsId)
                      onSubmitLike()
                    }}
                  >
                    <ThumbUpAlt sx={{color: process.env.REACT_APP_MAIN_COLOR}}/>
